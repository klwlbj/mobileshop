...
<section class="Body">
    <ul class="order_ul">
        ...
        <div style="margin-left: 1em">  <div><span> 物流公司：<?php echo ($info["expName"]); ?> </span><span style="float: right;">状态：
        <?php switch($info["deliverystatus"]): case "1": ?>在途中<?php break;?>
            <?php case "2": ?>正在派件<?php break;?>
            <?php case "3": ?>已签收<?php break;?>
            <?php case "4": ?>派送失败<?php break;?>
            <?php case "5": ?>查询失败<?php break; endswitch;?>
        </span> </div>  <p> 运单号码：<?php echo ($info["number"]); ?></p>  </div>
        <!--//"deliverystatus": "3",         /*  1.在途中 2.正在派件 3.已签收 4.派送失败  */-->
        <div>
            <ul>
                <?php if($info["deliverystatus"] != 5): ?><li style="padding: 1em;">
                    <p class="">物流详情</p>
                    <?php if(is_array($time)): $i = 0; $__LIST__ = $time;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="color"><div class=""><p class=""><?php echo ($vo["time"]); ?></p> <p class=""><?php echo ($vo["status"]); ?></p> </div> </div><?php endforeach; endif; else: echo "" ;endif; ?>
                </li><?php endif; ?>
            </ul>
        </div>
