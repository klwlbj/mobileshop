...
class IndexController extends CommonController {
    ...
    public function index(){
        ...
        foreach ($cate1 as $key => $value) {
            ...
            foreach ($cate2 as $kk => $vv) {
              $cate3=$cate->where(array('pid'=>$vv['id']))->select();
              $lm[$key]['cate'][$kk]['cate']=$cate3;


              //查询每个三级类目下面的商品
              foreach ($cate3 as $k => &$v) {
                $gs=$goods->field("id,goods_name,max_thumb,shop_price")->where(array('cate_id'=>$v['id']))->select();
                foreach ($gs as $a => &$b) {
                    $b['max_thumb']=substr($b['max_thumb'],1);
                }

                if($gs){
                    // $lm[$key]['cate'][$kk]['cate'][$k]['goods']=$goods->field("id,goods_name,max_thumb,shop_price")->where(array('cate_id'=>$v['id']))->select();
                    $go[]=$goods->field("id,goods_name,max_thumb,shop_price")->where(array('cate_id'=>$v['id']))->limit(9)->select();
                    // dump($go);
                }
              }
            ...
            foreach($go as $key=>$val){
                foreach($val as $k=>&$v){
                  ...
                  $v['max_thumb']=substr($v['max_thumb'],1);
                  $newArr[] = $v;
                }
            }
            ...
            $goods_res = $value;
            $goods_res['goods'] = $newArr;
            $goods_res['sjs']=rand(1,6);
            // dump($goods_res);
            $g_res[] = $goods_res;//商品列表
        }
 ...
 // dump($cnxh);
    ...
    public function detail(){
        ...
        $cate=D('cate');
        $cate1=$cate->where(array('id'=>$goods['cate_id']))->find();
        $cate2=$cate->where(array('pid'=>$cate1['pid']))->select();
